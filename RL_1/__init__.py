...
class Model:
    ...
    def createModel(self):
        ...
        self.model = tf.keras.Sequential([
            ...
            tf.keras.layers.Dense(50, input_dim=self._num_states, activation='relu'),
            tf.keras.layers.Dense(50, activation='relu'),
            tf.keras.layers.Dense(self._num_actions)
            ])
...
# Create a callback that saves the model's weights every 5 epochs
    ...
    def predict_batch(self, states_nodos,states_wp):
        ...
        lista = []
        for a in states:
            lista.append(self.predict_one(a))
        return lista

    def train_batch(self, x_batch_nodo, y_batch_nodo,x_batch_wp, y_batch_wp):
        self.model.fit( x_batch, y_batch, callbacks=[self.cp_callback])
...
class GameRunner:
    ...
    def run(self):
        ...
        while True:
            ...
            action_node,action_workPlace = self._choose_action(state_nodo, state_wp)
    ...
    def _choose_action(self, state, env):
        if random.random() < self._eps:
            return  env.select_random(state)
        else:
    ...
    def _replay(self):
        batch = self._memory.sample(self._model.batch_size) #devuelve el menor entre el tamaño de la info guardada y la cantidad que quiero pasar
        ...
        next_states_wp = np.array([(np.zeros(self._model.num_states_nodos)
                                 if val[4] is None else val[4]) for val in batch])
        # predict Q(s,a) given the batch of states
        q_s_a = self._model.predict_batch(states_nodos,states_wp)
        # predict Q(s',a') - so that we can do gamma * max(Q(s'a')) below
        q_s_a_d = self._model.predict_batch(next_states_nodos,next_states_wp)
        # setup training arrays
        x_nodo = np.zeros((len(batch), self._model.num_states_nodes))
        y_nodo = np.zeros((len(batch), self._model.num_actions_nodes))
        x_wp = np.zeros((len(batch), self._model.num_states_wp))
        y_wp = np.zeros((len(batch), self._model.num_actions_wp))
        
        for i, b in enumerate(batch):
            state_node,state_wp, action_node,action_wp, reward, next_state = b[0], b[1], b[2], b[3], b[4],b[5],b[6]
            # get the current q values for all actions in state
            current_q = q_s_a[i]
            # update the q value for action
            if (next_state is None) or i>=len(q_s_a_d):
                # in this case, the game completed after action, so there is no max Q(s',a')
                # prediction possible
                current_q[action_node,action_wp] = reward
            else:
                try:
                    current_q[action_node,action_wp] = reward + GAMMA * np.amax(q_s_a_d[i])
                except:    
                    current_q[action_node,action_wp] = reward 
            x_nodo[i] = state_node
            x_wp[i] = state_wp
